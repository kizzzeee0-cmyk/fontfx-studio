(() => {
  ...
  const state = {
    ...
    recentColors: { stampFill:'#FFFFFF', stampStroke:'#8FC8FF', decorFill:'#FFFFFF', decorStroke:'#8FC8FF', stroke:'#5C55A8' }
  };
  ...
  const RECENT_COLOR_STORAGE_KEY = 'fontfx.recentColors.v1';
  ...
  function loadRecentColors(){
    try{ const raw=JSON.parse(localStorage.getItem(RECENT_COLOR_STORAGE_KEY)||'{}'); if(raw&&typeof raw==='object') state.recentColors=Object.assign({},state.recentColors,raw); }catch(_){ }
  }
  function persistRecentColors(){ try{ localStorage.setItem(RECENT_COLOR_STORAGE_KEY,JSON.stringify(state.recentColors)); }catch(_){ } }
  function rememberRecentColor(key,value){ const hex=normalizeHexInput(value,state.recentColors[key]||'#FFFFFF'); state.recentColors[key]=hex; persistRecentColors(); return hex; }
  ...
  function stampShapeName(type){ return ({circle:'원',heart:'기본 하트',heartBasic:'기본 하트',heartChubby:'통통 하트',heartDoodle:'낙서 하트',heartRibbon:'리본 하트',star:'별',firework:'폭죽',sparkle:'반짝',bubble:'방울',flower:'꽃'}[type]||'스탬프'); }
  ...
  function drawStampShape(g,type,cx,cy,size){
    ...
    if(type==='heart' || type==='heartBasic'){
      ...
      const rays=12, innerR=r*0.30, outerR=r*0.90, rayW=Math.max(2.5,r*0.095);
      for(let i=0;i<rays;i++){
        const ang=-Math.PI/2 + i*(Math.PI*2/rays), ux=Math.cos(ang), uy=Math.sin(ang), px=-uy, py=ux;
        const a1={x:cx+ux*innerR,y:cy+uy*innerR}, a2={x:cx+ux*outerR,y:cy+uy*outerR}, wx=px*rayW, wy=py*rayW;
        g.moveTo(a1.x+wx,a1.y+wy); g.lineTo(a2.x+wx,a2.y+wy); g.quadraticCurveTo(a2.x+ux*rayW,a2.y+uy*rayW,a2.x-wx,a2.y-wy); g.lineTo(a1.x-wx,a1.y-wy); g.closePath();
      }
      g.moveTo(cx+r*0.15,cy); g.arc(cx,cy,r*0.15,0,Math.PI*2);
      for(let i=0;i<4;i++){ const ang=-Math.PI/4+i*Math.PI/2, px=cx+Math.cos(ang)*r*0.54, py=cy+Math.sin(ang)*r*0.54; g.moveTo(px+r*0.065,py); g.arc(px,py,r*0.065,0,Math.PI*2); }
    }else if(type==='flower'){
      for(let i=0;i<6;i++){
  ...
  function newStampObject(shape,x,y){
    const fill=normalizeHexInput(state.stampTool.fill,state.recentColors?.stampFill||'#FFFFFF');
    const strokeColor=normalizeHexInput(state.stampTool.strokeColor,state.recentColors?.stampStroke||'#8FC8FF');
  ...
  function defaultStroke(index=0){ const color=normalizeHexInput(state.recentColors?.stroke,index?'#FFFFFF':'#5C55A8'); return {id:uid('stroke'),enabled:true,width:index?8:14,position:'outside',blend:'normal',opacity:100,color}; }
  ...
  function wireOuterShadowItem(el,id){
    el.addEventListener('input',(ev)=>{
      ...
      if(k==='color'){ const hex=normalizeHex(v); const t=ev.target.parentElement.querySelector('[data-k="colorHex"]');if(t)t.value=hex; }
    });
  ...
  function wireEffectItem(el,type,id){
    el.addEventListener('input',(ev)=>{
      ...
      if(k==='color'){ const hex=rememberRecentColor('stroke',v); const t=ev.target.parentElement.querySelector('[data-k="colorHex"]');if(t)t.value=hex; }
    });
    el.addEventListener('change',(ev)=>{
      if(ev.target.dataset.k==='colorHex'){const hex=rememberRecentColor('stroke',ev.target.value);ev.target.value=hex;const cp=ev.target.parentElement.querySelector('[data-k="color"]');if(cp)cp.value=hex;applyEffectMutation(ch=>{const arr=type==='stroke'?ch.strokes:ch.innerShadows;const item=arr.find(x=>x.id===id);if(item)item.color=hex;});}
    });
    el.addEventListener('click',(ev)=>{
  ...
  function bindEvents(){
    ...
    $('fitBtn').addEventListener('click',fitPreview); $('centerSelectedBtn').addEventListener('click',centerSelected); $('centerXBtn').addEventListener('click',centerSelectedX); $('centerYBtn').addEventListener('click',centerSelectedY); ['drawModeBtn','drawModeBtn2'].forEach(id=>$(id).addEventListener('click',()=>{setDrawMode(!state.drawTool.enabled);})); ['paintModeBtn','paintModeBtn2'].forEach(id=>$(id).addEventListener('click',()=>{setPaintMode(!state.paintTool.enabled);})); $('stampModeBtn2')?.addEventListener('click',()=>setStampMode(!state.stampTool.enabled)); $('stampBaseSize')?.addEventListener('input',e=>{state.stampTool.size=Number(e.target.value)||96; updateStampToolUI();}); $('stampFillColor')?.addEventListener('input',e=>{state.stampTool.fill=rememberRecentColor('stampFill',e.target.value); if($('stampFillHex')) $('stampFillHex').value=state.stampTool.fill; updateStampToolUI();}); $('stampFillHex')?.addEventListener('change',e=>{state.stampTool.fill=rememberRecentColor('stampFill',normalizeHexInput(e.target.value,state.stampTool.fill||'#FFFFFF')); e.target.value=state.stampTool.fill; if($('stampFillColor')) $('stampFillColor').value=state.stampTool.fill; updateStampToolUI();}); $('stampStrokeColor')?.addEventListener('input',e=>{state.stampTool.strokeColor=rememberRecentColor('stampStroke',e.target.value); if($('stampStrokeHex')) $('stampStrokeHex').value=state.stampTool.strokeColor; updateStampToolUI();}); $('stampStrokeHex')?.addEventListener('change',e=>{state.stampTool.strokeColor=rememberRecentColor('stampStroke',normalizeHexInput(e.target.value,state.stampTool.strokeColor||'#8FC8FF')); e.target.value=state.stampTool.strokeColor; if($('stampStrokeColor')) $('stampStrokeColor').value=state.stampTool.strokeColor; updateStampToolUI();}); $('stampStrokeWidth')?.addEventListener('input',e=>{state.stampTool.strokeWidth=clamp(Number(e.target.value)||0,0,80); updateStampToolUI();}); document.querySelectorAll('#stampTypeGrid [data-stamp-type]').forEach(btn=>btn.addEventListener('click',()=>{state.stampTool.shape=btn.dataset.stampType||'circle'; setStampMode(true); updateStampToolUI();})); $('decorModeBtn2')?.addEventListener('click',()=>setDecorMode(!state.decorTool.enabled)); $('decorBrushType')?.addEventListener('change',e=>{state.decorTool.brushType=e.target.value||'pen'; updateDecorToolUI();}); $('decorAssistMode')?.addEventListener('change',e=>{state.decorTool.assistMode=e.target.value||'freehand'; updateDecorToolUI();}); $('decorColor')?.addEventListener('input',e=>{state.decorTool.color=rememberRecentColor('decorFill',e.target.value); if($('decorColorHex')) $('decorColorHex').value=state.decorTool.color; updateDecorToolUI();}); $('decorColorHex')?.addEventListener('change',e=>{state.decorTool.color=rememberRecentColor('decorFill',normalizeHexInput(e.target.value,state.decorTool.color||'#FFFFFF')); e.target.value=state.decorTool.color; if($('decorColor')) $('decorColor').value=state.decorTool.color; updateDecorToolUI();}); $('decorOutlineColor')?.addEventListener('input',e=>{state.decorTool.outlineColor=rememberRecentColor('decorStroke',e.target.value); if($('decorOutlineHex')) $('decorOutlineHex').value=state.decorTool.outlineColor; updateDecorToolUI();}); $('decorOutlineHex')?.addEventListener('change',e=>{state.decorTool.outlineColor=rememberRecentColor('decorStroke',normalizeHexInput(e.target.value,state.decorTool.outlineColor||'#8FC8FF')); e.target.value=state.decorTool.outlineColor; if($('decorOutlineColor')) $('decorOutlineColor').value=state.decorTool.outlineColor; updateDecorToolUI();}); $('decorOutlineWidth')?.addEventListener('input',e=>{state.decorTool.outlineWidth=clamp(Number(e.target.value)||0,0,80); updateDecorToolUI();}); $('decorSize')?.addEventListener('input',e=>{state.decorTool.size=Math.max(1,Number(e.target.value)||14); updateDecorToolUI();}); $('decorStabilize')?.addEventListener('change',e=>{state.decorTool.stabilize=clamp(Number(e.target.value)||0.88,0,0.98); updateDecorToolUI();});
  ...
  async function boot(){loadRecentColors();loadEffectPresets();loadPaintPresets();loadFontTransformPolicies();initFonts();renderPalettes();renderHarmony(makeHarmony('#9389DE'));renderAllEffectPresetControls();renderPaintPresetControls();bindEvents();resizeDisplay();drawEyedropperPlaceholder();setEyedropperSample(state.eyedropper.sample||'#9389DE'); if(state.eyedropper.imageDataUrl){ try{ await loadEyedropperDataUrl(state.eyedropper.imageDataUrl,'복원된 예시 사진'); }catch(_){ drawEyedropperPlaceholder(); } } pushHistory();updateAll();await restoreSavedFonts();updateAll();}
  boot();
})();
